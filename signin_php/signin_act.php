...
//販売員かユーザーの判定
if($attribute == 0){
    $sql = 'SELECT * FROM user WHERE email=:email AND password=:password AND is_deleted=0';
}elseif($attribute == 1){
  ...
  exit();
    ...
    if(!$val){
        ...
        if($attribute == 0){
            ...
            $_SESSION['uid'] = $val['id'];
            header('Location:../main_user_php/user.php');
            exit();
        }elseif($attribute == 1){
            $_SESSION = array();
            $_SESSION['session_id'] = session_id();
            $_SESSION['name'] = $val['name'];
            $_SESSION['uid'] = $val['id'];
            header('Location:../main_salesperson_php/salesperson.php');
            exit();
        }
